...
This notebook develops an example of a spatial (maize) yield model. The approach consists of a combination of a standard [Production function (PF)](https://en.wikipedia.org/wiki/Production_function) that considers outputs relative to inputs and a novel **Site quality index (SI)**, which describes the quality of the production environment for the purpose of growing a particular crop. The combined model can be expressed as: **y ~ f (PF, SI)**, and the explicit version of this can be used to identify the potential productivity of croplands and to provide a comparative frame of reference for management options.
...
Note however, that you are not limited to only these specific models. The [caret]() package, on which we rely on here, offers many alternatives. You may want to explore and experiment with those.
...
The last model we fit here is a simple feed-forward neural network.
