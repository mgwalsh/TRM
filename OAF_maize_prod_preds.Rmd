...
boxplot(yield~qy, notch=T, ylab="Maize yield (t/ha)", xlab="SI label", cex.lab=1.4, gsdat)
```
...
There are numerous ways to quantify the uncertainty inherent in these predictions. We take a simple but quite robust approach here using quantile regression with ([quantreg](https://cran.r-project.org/web/packages/quantreg/quantreg.pdf)). We are mainly interested in the initial spread of the ROI-wide predictions (sensu, their 95% probable intervals). Once new data are collected, we shall be revising these and will also point out some additional (more spatially data sensitive) techniques.
