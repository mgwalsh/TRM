...
Note that you are not limited to only the models that we use here. The [`caret`](https://topepo.github.io/caret/index.html) package, on which we rely on here, offers many compelling alternatives. We encourage you to explore and experiment with those.
...
We use a [Multilevel model](https://en.wikipedia.org/wiki/Multilevel_model) to combine the predicted SIs with the previously presented minimal production function using the [`arm`](https://www.rdocumentation.org/packages/arm/versions/1.11-2) package. In this particular example we specify a random intercept model with in which the intercepts are allowed to vary by year and administrative location.
...
Finally, we write out the complete dataframe, including all of the predictions for reference, reproducability and further use. 
